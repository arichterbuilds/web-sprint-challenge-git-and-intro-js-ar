...
function removeArtist(artists, n1) {
  const nArr = artists
  nArr.splice(n1, 1)
  return nArr
}
...
function addArtist(artists, name, years, genre, nationality, bio) {
  const nArr = artists
  nArr.push({name, years, genre, nationality, bio})
  return nArr
}
...
function artistInfo(artists, aname){
  for(let i = 0; i < artists.length; i++){
    if(artists[i].name === aname){
